...
- [x] Web用户界面
- [x] 修复模拟代理合约代码，增加安全性
- [x] 内置默认支持主流链以及测试链：ethereum，sepolia，polygon，optimisum，op_sepolia，arbitrum，base，bsc，内置配置好url和chainname等固定参数，但是支持用户自定义url
- [x] 支持浏览器缓存用户输入的私钥
- [x] 流程优化：1. 输入账户和gas私钥(可选) 2. 默认使用sepolia链，左上角可以选择链 3. 在下拉选择链的下拉框中增加“自定义链”功能，可以增加自定义网络，输入链名字和rpc即可 4. 主页：4.1 交易页面：发送交易erc20和native token合并为发送交易页面 4.2 7702授权管理：绑定和解绑都在这里 4.3 工具类：目前的“Mint ERC20 Token”页面
- [ ] 交易历史记录：使用本地缓存的记录就可以
- [ ] Gas估算优化
